...
@section('content')
        ...
        <div class="relative rounded overflow-hidden px-6 sm:px-8 md:px-12 lg:px-16 xl:px-20 py-8 sm:py-10 md:py-12 lg:py-16 flex flex-col lg:flex-row items-center justify-between gap-6 sm:gap-8 lg:gap-10"
            ...
            <div class="relative z-10 w-full lg:max-w-lg text-center lg:text-left">
                ...
                <p class="text-white/80 text-sm sm:text-base leading-relaxed mb-5 sm:mb-6 md:mb-7 max-w-md mx-auto lg:mx-0">
                    Unlock {{ $totalCourses ?? 0 }}+ top courses — anytime, anywhere. Empower your workforce with expert-led
                    learning.
                </p>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-3 sm:gap-3.5">
                @foreach ($topCategories as $topCategory)
                    <a href="{{ route('courses.search', ['category' => $topCategory->slug]) }}">
                        <div data-reveal
                            class="bg-white border  rounded p-3 sm:p-4 cursor-pointer hover:border-purple-400 hover:shadow-[0_4px_16px_rgba(164,53,240,0.12)] hover:-translate-y-0.5 transition-all duration-200">

                            {{-- Icon --}}
                            <div class="text-xl sm:text-2xl mb-2 sm:mb-2.5 text-center">
                                {{ $topCategory->icon }}
                            </div>

                            {{-- Category Name --}}
                            <div class="font-semibold text-xs sm:text-sm  leading-snug text-center line-clamp-2">
                                {{ $topCategory->name }}
                            </div>

                            {{-- Course Count --}}
                            <div class="text-[10px] sm:text-[11px]  mt-1 text-center">
    ...
    <section class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-8 sm:py-10 md:py-14">
        <div class="flex flex-col sm:flex-row sm:items-baseline justify-between gap-3 mb-6 sm:mb-7">
            ...
            <a href="{{ route('index') }}" class="text-purple-600 text-sm font-semibold hover:underline">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            @foreach ($topInstructors as $instructor)
                <div data-reveal
                    ...
                    {{-- Avatar --}}
                    <div
                        class="w-16 h-16 sm:w-20 sm:h-20 rounded-full mx-auto mb-3 sm:mb-4 flex items-center justify-center text-xl sm:text-2xl font-bold text-white bg-gradient-to-br from-purple-500 to-violet-700 to-cyan-800">
                        {{ $instructor->initials() }}

                    </div>

                    {{-- Name --}}
                    <div class="font-bold text-sm sm:text-base  mb-1">{{ $instructor->name }}</div>

                    {{-- Title --}}
                    <div class="text-xs  mb-3 sm:mb-4 line-clamp-2">{{ $instructor->email }}</div>

                    {{-- Stats --}}
                    <div class="flex justify-center gap-3 sm:gap-4 md:gap-5 text-xs">
                        <div class="text-center">
                            <strong class="block text-sm ">
                                {{ number_format($instructor->course_reviews_avg_rating ?? 0, 1) }}★</strong>
                            <span class="">Rating</span>
                        </div>
                        <div class="text-center">
                            <strong class="block text-sm ">{{ $instructor->students_count }}</strong>
                            <span class="">Students</span>
                        </div>
                        <div class="text-center">
                            <strong class="block text-sm ">{{ $instructor->courses_count }}</strong>
    ...
    <section class="max-w-7xl mx-auto px-12 py-14">
        <div class="text-center mb-10">
            <h2 class="font-sora text-2xl font-bold ">What Our Learners Say </h2>
        ...
        <div
            ...
            <div class="max-w-xl w-full text-center lg:text-left">
                ...
                <p class="text-white/70 text-sm sm:text-base leading-relaxed mb-6 sm:mb-8 max-w-md mx-auto lg:mx-0">
                    Join {{ $instructor->count() }}+ instructors teaching on SkillNest. Set your own schedule, reach millions of learners
                    globally,
