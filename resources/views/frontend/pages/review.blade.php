    ...
    <div class="min-h-screen bg-gray-50">
                ...
                <div class="flex items-start gap-4 sm:gap-6">
                    ...
                    <div class="hidden sm:block shrink-0">
                        <img src="{{ $course->getFirstMediaUrl('thumbnail') ?: '🤖' }}" alt="{{ $course->title }}"
                            class="w-24 sm:w-32 h-16 sm:h-20 object-cover rounded border border-gray-200">
                    </div>

                    {{-- Course Info --}}
                    <div class="flex-1 min-w-0">
                        <h1 class="font-sora text-xl sm:text-2xl font-bold">
                            {{ $course->title }}
                        </h1>
                        <p class="text-sm sm:text-base text-gray-600">
                            {{ $course->description ? Str::limit($course->description, 160) : 'No description available.' }}
        ...
        <div class="max-w-4xl mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8 md:py-10">
            ...
            <form action="{{ route('reviews.store', $course->id) }}" method="POST" class="space-y-6 sm:space-y-8">
                ...
                {{-- Rating Section --}}
                <div class="bg-white rounded border border-gray-200 p-5 sm:p-6 md:p-8">
                    ...
                    <div class="flex flex-col sm:flex-row items-start sm:items-center gap-4 sm:gap-6">
                        ...
                        <div class="flex gap-3 w-full sm:w-auto">
                            <a href="{{ route('my-learning.index') }}"
                                class="flex-1 sm:flex-none px-6 sm:px-7 py-3 border-2 border-gray-300 text-gray-700 font-semibold rounded hover:bg-gray-50 transition text-center text-sm sm:text-base">
                                Cancel
                            </a>
                            <button type="submit"
            ...
            {{-- Guidelines Card --}}
            {{-- <div class="bg-blue-50 border border-blue-200 rounded p-5 sm:p-6 mt-6 sm:mt-8">
