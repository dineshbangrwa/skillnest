    ...
    <div class="bg-gray-900 text-white py-8 sm:py-10 px-4 sm:px-8 md:px-12">
        <div class="max-w-7xl mx-auto">
            ...
            <div class="flex items-center gap-2 text-xs text-white/60 mb-4 flex-wrap">
                ...
                <span>/</span>
                <a href="{{ route('courses.search') }}" class="hover:text-white transition">Development</a>
                {{-- <span>/</span> --}}
            ...
            <div class="flex flex-col lg:flex-row gap-8">
                <div class="flex-1">
                    ...
                    <div class="flex items-center gap-3 mb-3 flex-wrap">
                        <span
                            class="text-amber-400 font-bold text-lg">{{ number_format($course->average_rating ?? 0, 1) }}</span>
                        <span class="text-amber-400">★★★★★</span>
                        <span class="text-white/60 text-sm underline cursor-pointer">
                            ({{ $course->reviews_count ?? 0 }})</span>
                ...
                <div class="lg:hidden border border-white/20 rounded overflow-hidden bg-white/5 backdrop-blur">
                    ...
                    @if ($previewLesson && $previewLesson->getFirstMediaUrl('video'))
                        <img src="{{ $course->getFirstMediaUrl('thumbnail') }}" alt="{{ $course->title }}"
                            class="w-full h-full object-cover absolute inset-0">
                        <div class="absolute inset-0 flex items-center justify-center bg-black/20 z-10">
                            <div class="w-14 h-14 rounded-full bg-white/20 backdrop-blur flex items-center justify-center cursor-pointer hover:bg-white/30 transition"
                                onclick="openVideoModal('{{ $previewLesson->getFirstMediaUrl('video') }}')">
                    ...
                    <div class="p-5">
                        ...
                        @elseif($inCart)
                            ...
                            <button type="button" onclick="window.location.href='{{ route('buy.index', $course->slug) }}'"
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 md:px-8 py-8 flex flex-col lg:flex-row gap-8 items-start">
        ...
        <div class="hidden lg:block w-80 shrink-0">
            <div class="sticky top-20 border border-gray-200 rounded overflow-hidden shadow-xl">
                <div class="h-44 bg-gradient-to-br from-violet-500 to-purple-800 relative overflow-hidden">
                    @if ($previewLesson && $previewLesson->getFirstMediaUrl('video'))
                        <img src="{{ $course->getFirstMediaUrl('thumbnail') }}" alt="{{ $course->title }}"
                            class="w-full h-full object-cover absolute inset-0">
                        <div class="absolute inset-0 flex items-center justify-center bg-black/20 z-10">
