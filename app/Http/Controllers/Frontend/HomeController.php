...
class HomeController extends Controller
{
    ...
    public function index()
    {
        $topCategories = Cache::remember('home_top_categories', 3600, function () {
            return Category::where('status', 1)
                ->withCount('courses')
                ->orderBy('courses_count', 'desc') // 👈 important
                ->limit(6)
                ->get();
        });
        ...
        return view('frontend.pages.home', compact('bestSellingCourses', 'topInstructors', 'topCategories', 'totalCourses', 'reviews'));
    }
    ...
    public function course($slug)
    {
        ...
        $previewLesson = $course->sections
            ->flatMap->lessons
            ->firstWhere('is_preview', true);

        return view('frontend.pages.course.detail', compact('course', 'isPurchased', 'inCart', 'previewLesson'));
    }
